...
import { PelanggaranFormData } from "@/types/pelanggaran";
import { Siswa } from "@/types/siswa";
import PelanggaranForm from "./PelanggaranForm";

interface TambahPelanggaranModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSave: (data: PelanggaranFormData) => void;
  siswaList?: Siswa[];
}

const today = new Date().toISOString().split("T")[0];

const EMPTY_FORM: PelanggaranFormData = {
  namaSiswa: "",
  nis: "",
  kelas: "",
  jenisPelanggaran: "",
...
export default function TambahPelanggaranModal({
  isOpen,
  onClose,
  onSave,
  siswaList,
}: TambahPelanggaranModalProps) {
  const [formData, setFormData] = useState<PelanggaranFormData>(EMPTY_FORM);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!formData.namaSiswa.trim()) { setErrorMsg("Siswa wajib dipilih / diisi!"); return; }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 backdrop-blur-xs p-4 animate-in fade-in duration-200">
      ...
      >
        ...
        <div className="flex items-start justify-between p-6 pb-2 flex-shrink-0">
          <div>
            <h3 className="text-lg font-bold text-slate-800">Tambah Pelanggaran</h3>
            <p className="text-xs text-slate-500 mt-0.5">Silakan masukkan data pelanggaran siswa.</p>
          </div>
          <button
            onClick={handleClose}
            type="button"
